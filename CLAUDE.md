...
- **`CDK_DEFAULT_REGION` can't be used to control the deploy region via shell export.** The CDK CLI unconditionally overwrites it right before spawning `bin/app.ts`, using the AWS SDK's own default-region resolution — which falls back to `us-east-1` with no credentials configured, clobbering whatever you exported. (`CDK_DEFAULT_ACCOUNT` is unaffected — the CLI only sets it when a real account lookup succeeds, so it's fine to drive from CI/local env.) `bin/app.ts` hardcodes the deploy region (`us-west-2`) directly for this reason; `cdk.context.json` caches the AZ lookups for both `us-west-2` (main stacks) and `us-east-1` (`BudgetsStack`, pinned there since billing metrics only publish in that region) so `cdk synth` works without live credentials.
