# Fully offline via mock_provider: terraform test never reaches real AWS here, matching CDK synth's own
# zero-credential guarantee. Values below are arbitrary but well-formed stand-ins for the real ones a deploy
# would pass — see infra/variables.tf for what each means.
mock_provider "aws" {}
...
run "default_security_group_is_stripped" {
  ...
  assert {
    ...
    error_message = "the VPC's default security group must carry no rules (equivalent of CDK's restrictDefaultSecurityGroup)"
  }
}
