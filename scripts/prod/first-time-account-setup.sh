#!/usr/bin/env bash
# One-time setup the root module can't do for itself: the Clerk secret, the Spot service-linked role, and the Terraform
# state bucket. Safe to re-run. Anything that already exists is kept, and the Clerk secret's value is never overwritten.
...
confirm "Create the Clerk secret, the Spot service-linked role, and $BUCKET in account $AWS_ACCOUNT_ID?"
...
if ! SECRET_STATE=$(aws secretsmanager describe-secret --region us-west-2 --secret-id "$SECRET_NAME" \
...
if [[ $SECRET_STATE == None ]]; then
  ...
  aws secretsmanager restore-secret --region us-west-2 --secret-id "$SECRET_NAME" >/dev/null
  ...
  SECRET_ARN=$(printf '%s' "$CLERK_SECRET_KEY" | aws secretsmanager create-secret --region us-west-2 \
...
aws secretsmanager tag-resource --region us-west-2 --secret-id "$SECRET_NAME" \
