...
#
# .github/workflows/deploy.yml signs its AWS credentials with tf_aws_region and smoke-tests the origin tf_app_hostname
# builds, so a spelling in infra/variables.tf or infra/locals.tf that these no longer read breaks a deploy rather than
# a plan. The checks against the real files are shape-only, since asserting the region or the hostname literally would
# write each a second time. The fixtures below own their inputs, so those assert exact output.
...
HCL
check_equals "tf_var_default skips a validation block" "eu-central-1" "$(tf_var_default aws_region)"
check_equals "tf_aws_region takes that default" "eu-central-1" "$(tf_aws_region)"
...
HCL
check_fails "tf_var_default stops at the variable's closing brace" tf_var_default aws_region

# An expression rather than a literal. The awk leaves a stray token here, which is the reason tf_aws_region checks the
# shape of what it got instead of passing it straight to the AWS CLI.
cat > "$FIXTURE/infra/variables.tf" <<'HCL'
variable "aws_region" {
  default = format("%s-%s", "us", "west-2")
}
HCL
check_fails "tf_aws_region refuses a default it can't read as a region" tf_aws_region
...
HCL
check_equals "tf_app_hostname substitutes the zone name" "splat.example.com" "$(tf_app_hostname example.com)"
...
HCL
check_fails "tf_app_hostname refuses a hostname it only partly resolved" tf_app_hostname example.com
