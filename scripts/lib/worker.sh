...
# ai-gaussian-splatter-dev IAM user the container does, and run_worker_stage passes the same values in. Stops the script
# if AWS rejects the keys.
use_dev_aws_credentials() {
  if [[ ! -f "$ROOT/web/.env" ]]; then
    echo "web/.env is missing. Run scripts/dev/create-dev-resources.sh to create it." >&2
    exit 1
  fi
  ...
  AWS_ACCESS_KEY_ID=$(get_env_var "$ROOT/web/.env" AWS_ACCESS_KEY_ID)
  AWS_SECRET_ACCESS_KEY=$(get_env_var "$ROOT/web/.env" AWS_SECRET_ACCESS_KEY)
  AWS_REGION=$(get_env_var "$ROOT/web/.env" AWS_REGION)
  UPLOADS_BUCKET=$(get_env_var "$ROOT/web/.env" UPLOADS_BUCKET)
  SPLATS_BUCKET=$(get_env_var "$ROOT/web/.env" SPLATS_BUCKET)
  ...
  if ! aws sts get-caller-identity >/dev/null 2>&1; then
    ...
    echo "Then run scripts/dev/create-dev-resources.sh to create and write a new one." >&2
    exit 1
  fi
}
...
# Usage: run_worker_stage <splat-id> <stage> [extra podman run args]
#
# Call use_dev_aws_credentials first. A full run is two container runs, one per STAGE, because in production each stage
