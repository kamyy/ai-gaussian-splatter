...
> 🚧 **Under construction.** The pipeline has run end to end on a local GPU, but nothing has run on AWS yet and several gaps remain — see [State / what's next](AGENTS.md#12-state--whats-next), [`ARCHITECTURE.md`](ARCHITECTURE.md), and [`RUNBOOK.md`](RUNBOOK.md).
...
Quality depends on angular coverage and overlap, not raw count. Aim for **~50 well-spaced** views (every side, a couple of heights, neighboring shots overlapping) rather than many near-duplicates. Capture tips: [`RUNBOOK.md`](RUNBOOK.md#42-capture).
...
`pnpm dev` needs a local Postgres and a filled-in `.env` first — see [`RUNBOOK.md`](RUNBOOK.md#2-web-frontend--rest-api).
