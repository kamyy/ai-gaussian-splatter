...
Rejected: a CloudFormation custom resource (Lambda-backed) gating the service on migration success within one `cdk deploy`. Technically tighter (one deploy call, CloudFormation-native sequencing) but trades a plain, linear GitHub Actions log — every step a visible `aws`/`cdk` command — for a Lambda whose failure mode is debugged through a different service's logs. Also rejected: running migrations from a human's laptop through a bastion (today's fully-manual fallback, kept in "Applying migrations to a deployed database" for out-of-band fixes) — works, but isn't something CI can do unattended.
...
CI authenticates to AWS via **GitHub OIDC**, not static IAM access keys — no long-lived credential to leak or rotate, scoped by the identity token's `sub` claim to `repo:<owner>/<repo>:ref:refs/heads/main` specifically (PRs and forks can't assume the role). The role itself is created by hand once (RUNBOOK.md), not by a CDK stack: it's chicken-and-egg (CI can't deploy the stack that grants CI its own deploy permission) and account-wide/security-sensitive, the same reasoning that already keeps the Clerk secret and the `AWSServiceRoleForEC2Spot` check as hand-run, RUNBOOK-documented one-time setup rather than stack-managed resources. `cdk deploy` itself runs under that role via `sts:AssumeRole` on CDK's own bootstrap roles (created by `cdk bootstrap`), rather than granting the CI role broad CloudFormation/IAM permissions directly.
