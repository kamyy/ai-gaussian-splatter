...
**pnpm**, not npm or yarn, for both `web/` and `infra/` (the latter only needs it for the npm-distributed CDK CLI — `AGENTS.md`). Its content-addressable store keeps one copy of each package version on disk, hard-linked into every project that needs it, rather than duplicating packages per `node_modules`. Its `node_modules` layout also only exposes packages a project actually lists in `package.json` — code can't accidentally import a transitive dependency that isn't declared, the "phantom dependency" problem npm's and yarn's flat layout allows.
...
2. **COLMAP** (`worker/pipeline/sfm.py`): exhaustive matching → camera poses + sparse cloud. Accuracy over speed for a small object-centric set. `worker/run_job.py` fails below 50% registered images — capture quality, not a pipeline bug.
...
TLS is required only where RDS enforces it (`rds.force_ssl = 1`), not by this app's own client code. `databaseSsl()`/`resolveDatabaseUrl()` (`web/lib/server/databaseUrl.ts`) make it conditional on `DATABASE_SSL_CA` being set. Local dev and CI run a plain, un-TLS'd Postgres. A per-environment self-signed CA (a custom Postgres image locally, a freshly generated cert on every CI run) was tried and dropped: the setup cost — bind-mounting cert/key pairs, hardening `pg_hba.conf`, keeping SANs in step with every hostname a client connects as — outweighed the benefit of exercising `web/lib/server/databaseUrl.ts`'s TLS-handshake-and-CA-verification branch outside of a real AWS deploy, which remains the only place that branch runs at all.

CI's Postgres starts as a plain `podman run` step (`.github/workflows/ci.yml`'s `web` job) rather than GitHub Actions' declarative `services:` block: the migrator-image test below needs to reach it by container name from a sibling podman container, and a Docker-managed `services:` container isn't reachable that way. A dedicated podman network, not `--network host` — host networking doesn't reliably provide true loopback under rootless podman here, verified directly against this runner setup.
...
RDS is `PRIVATE_ISOLATED` (no outbound need). Type must be explicit — `PRIVATE_WITH_EGRESS` + `nat_gateways=0` synthesizes "isolated in everything but name." Both security groups live in `infra/stacks/network_stack.py`; declaring the ALB group in `WebStack` makes the ingress rule cross-stack → `DependencyCycle`.

TLS terminates at the ALB (ACM cert for `ai-gaussian-splatter.orky.net`; 80→443). Cert is in `infra/stacks/web_stack.py` so it lands in the ALB's region — ALBs can't use out-of-region certs; `us-east-1` matters only for CloudFront (unused).
...
The web image is tagged per release with the commit SHA, in an ECR repository this app owns (`infra/stacks/registry_stack.py`), and the tag travels as CDK context. A moving tag like `latest` would be simpler to push but leaves every release sharing one task definition, which disarms the deployment circuit breaker: rollback restarts the previous deployment against that same string, so Fargate re-pulls whatever was pushed most recently — the image that just failed. Per-release tags make each deploy its own task definition. The repository is also `IMMUTABLE`, so a pushed tag can never be repointed. Costs: a context value on every `cdk` invocation, a rebuild of an already-pushed commit is rejected at push time, and the rollback window is bounded by `RELEASES_KEPT` rather than unlimited. Rejected `ContainerImage.from_asset`, which solves the same problem by having CDK build and publish into the bootstrap asset repository. It removes the registry stack and the push step entirely, but moves the images out of a repository the app stacks own.
...
The core ordering problem: a migration must finish before any task running the *new* image starts serving traffic, but `ecs:RunTask` can only run an already-registered task-definition revision, and a single `cdk deploy` that updates both "the migration task's image" and "the service's image" together gives CloudFormation no place to pause between them. Solved by giving the migration task its own CDK context flag (`migrateImageTag`, defaulting to `webImageTag` so every existing manual invocation is unaffected) and calling `cdk deploy` twice: once to register the migration task against the *new* image while the service stays pinned to the *old* one (no diff, no rollout), then — only if the migration task exits 0 — again with both flags equal, which is what actually moves the service. CDK stays the sole owner of "what's currently deployed"; nothing calls `aws ecs update-service` out of band.

Rejected: a CloudFormation custom resource (Lambda-backed) gating the service on migration success within one `cdk deploy`. Technically tighter (one deploy call, CloudFormation-native sequencing) but trades a plain, linear GitHub Actions log — every step a visible `aws`/`cdk` command — for a Lambda whose failure mode is debugged through a different service's logs. Also rejected: running migrations from a human's laptop through a bastion. `DataStack`'s RDS instance sits in an isolated subnet with no NAT gateway and no security-group path for an ad hoc host, and no bastion exists in this infra, so there is no manual fallback — a bad migration is fixed the same way as any other bug, with a corrective migration through a normal PR ([RUNBOOK.md § "Fixing a bad migration"](RUNBOOK.md#fixing-a-bad-migration)).

A rolled-back *service* deployment does not undo an already-applied migration. Rollback and "was the migration a good idea" are orthogonal once the migration has committed. This is why every migration has to follow the expand/contract discipline in `AGENTS.md`, not an incidental style preference.

### CI authentication

CI authenticates to AWS via **GitHub OIDC**, not static IAM access keys — no long-lived credential to leak or rotate, scoped by the identity token's `sub` claim to `repo:<owner>@<ownerId>/<repo>@<repoId>:ref:refs/heads/main` specifically (PRs and forks can't assume the role). The role itself is created by hand once (RUNBOOK.md), not by a CDK stack: it's chicken-and-egg (CI can't deploy the stack that grants CI its own deploy permission) and account-wide/security-sensitive, the same reasoning that already keeps the Clerk secret and `AWSServiceRoleForEC2Spot` as hand-run, RUNBOOK-documented one-time setup rather than stack-managed resources. `cdk deploy` itself runs under that role via `sts:AssumeRole` on CDK's own bootstrap roles (created by `cdk bootstrap`), rather than granting the CI role broad CloudFormation/IAM permissions directly.
