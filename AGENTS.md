...
- **Node version is pinned in root `.nvmrc` (`24.18.0`), and `frontend`/`infra` CI jobs read that same file via `setup-node`'s `node-version-file` input** (2026-08-01) — instead of a separately hardcoded `node-version:`, so local dev and CI can't silently drift onto different majors. Run `nvm use` from repo root to match. Originally pinned to 24 specifically for its native TypeScript type-stripping (run `.ts` files directly, no `ts-node`/`tsx`), which root `scripts/*.ts` relied on — as of 2026-08-03 those scripts are plain `.js` (see the Biome-migration TS-pin bullet above for the same kind of re-evaluation), and nothing else in the repo executes `.ts` directly via Node (`frontend/`'s TS handling goes through Next.js/Vitest/Playwright's own transforms, none of which need it). The pin stays regardless: Node 24 is the current Active LTS release (supported through April 2028), which is reason enough on its own.
...
Note: `npx biome ci .` above validates the same thing CI checks, but not from inside CI's `frontend` job — it's a separate root-scoped `lint-format` job (single `biome ci .` run from repo root, covering `scripts/*.js` too, not just `frontend/`). The `frontend` job itself only runs `tsc`/`vitest`/`next build`/`playwright`.
