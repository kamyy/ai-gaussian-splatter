...
- **`next-themes`' `ThemeProvider` (`web/app/layout.tsx`) owns `[data-theme]`, with its own pre-hydration script setting the attribute before React hydrates.** `web/app/layout.tsx`'s `<html suppressHydrationWarning>` is required for that: the attribute the script sets deliberately doesn't match what the server rendered, and removing `suppressHydrationWarning` turns that expected mismatch into a hydration warning on every page load.
...
- **notistack's `SnackbarProvider` is wrapped in `web/components/layout/AppSnackbarProvider.tsx` because notistack doesn't ship `"use client"`.** Rendering it straight from `web/app/layout.tsx`, a Server Component, fails. The other providers there (`next-themes`' `ThemeProvider`, `ClerkProvider`) are rendered directly and need no wrapper.
...
- **Only the controls floating over the 3D viewer (`web/components/splats/SplatStageViewer.tsx`) are raised, through the `raised` (static) and `raised-button` (deeper shadow on hover, inset on press) utilities in `web/app/globals.css`.** Every other button and pill in the app stays flat. The utilities lay a sheen over the element's own background color, so each element still needs a `bg-*` class. `raised-button` also needs a `transition` class covering `box-shadow`.
- **Three layers share one flat z-index scale, since Tailwind has no built-in `zIndex` scale to reach for.** `z-1100` is the sticky site header, `z-1200` is a tooltip (`web/components/ui/Tooltip.tsx`), and `z-1300` is the dialog overlay/content. Slot a new floating layer in between the two it needs to sit between, not at the top of the scale by default.
