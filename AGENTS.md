...
Note: `npx biome ci .` above validates the same thing CI checks, but not from inside CI's `frontend` job — it's a separate root-scoped `lint-format` job (single `biome ci .` run from repo root, covering `scripts/*.ts` too, not just `frontend/`). The `frontend` job itself only runs `tsc`/`vitest`/`next build`/`playwright`.
