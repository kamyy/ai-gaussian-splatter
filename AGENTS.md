...
Was four packages until 2026-08-03, when the FastAPI `backend/` was folded into
the Next.js app and `frontend/` was renamed `web/` — see `docs/ARCHITECTURE.md`'s
dated note under "API & data" for why. Server-only code lives in `web/lib/server/`;
never import it from a `"use client"` file, or Prisma and the AWS SDK end up in
the browser bundle.
...
- **Next.js 16 renamed `middleware.ts` → `proxy.ts`** (function name `middleware` → `proxy` too). `@clerk/nextjs`'s `clerkMiddleware()` still works, just needs to live in `proxy.ts` now. **It must sit at the package root, beside `app/`, not inside it** — this repo had it at `app/proxy.ts` from 2026-07-30 until 2026-08-03, where Next silently never loaded it and `clerkMiddleware()` never ran. Nothing failed at the time because auth was enforced by the separate FastAPI service; the moment auth moved into Next, every authenticated route 500'd with "clerkMiddleware() was not run". A misplaced proxy file fails silently — verify `ƒ Proxy (Middleware)` appears in `next build`'s route table.
- **A structurally invalid Clerk publishable key now breaks the build/tests, where it used to be harmless.** CI's old dummy (`pk_test_ZmFrZS1rZXktZm9yLWNpJA`, base64 `"fake-key-for-ci$"`) doesn't parse as a Clerk key; it only worked because the middleware never ran. It's now `pk_test_ZXhhbXBsZS5jbGVyay5hY2NvdW50cy5kZXYk` (base64 `"example.clerk.accounts.dev$"`), which parses offline without contacting Clerk.
...
- **`prisma generate` must re-run after every schema edit** or the client keeps the old mapping — a renamed `@@map` on an enum produced `type "public.object_status" does not exist` at runtime while `tsc` stayed green. `postinstall` covers installs; re-run it by hand after editing `schema.prisma`.
...
- **Path params must be UUID-checked before they reach Prisma.** The id columns are `@db.Uuid`, so `/api/v1/gallery/abc` makes Postgres raise `22P02` and surfaces as a 500 — FastAPI got this free from `object_id: uuid.UUID`. Use `requireUuid()` (routes) or `isUuid()` (`lib/server/data.ts`, which returns null so pages can `notFound()`).
...
- **Server Components reading request-time data need `export const dynamic = "force-dynamic"`** — otherwise `next build` tries to statically prerender them. Until 2026-08-03 the failure mode was `ECONNREFUSED` against the separate backend; those pages now call `web/lib/server/data.ts` directly (a server fetching its own HTTP endpoint is pure overhead), so the directive stays for freshness rather than reachability.
...
- **No real Postgres available in a typical dev sandbox** (though `podman run postgres:18` worked in the 2026-08-03 session — try it before assuming otherwise) — `web/lib/server/rateLimit.test.ts` is real but skips unless `TEST_DATABASE_URL` is set (CI wires this via a Postgres service container; see `.github/workflows/ci.yml`).
- **Node version is pinned in root `.nvmrc` (`24.18.0`), and `web`/`infra` CI jobs read that same file via `setup-node`'s `node-version-file` input** (2026-08-01) — instead of a separately hardcoded `node-version:`, so local dev and CI can't silently drift onto different majors. Run `nvm use` from repo root to match. Originally pinned to 24 specifically for its native TypeScript type-stripping (run `.ts` files directly, no `ts-node`/`tsx`), which root `scripts/*.ts` relied on — as of 2026-08-03 those scripts are plain `.js` (see the Biome-migration TS-pin bullet above for the same kind of re-evaluation), and nothing else in the repo executes `.ts` directly via Node (`web/`'s TS handling goes through Next.js/Vitest/Playwright's own transforms, none of which need it). The pin stays regardless: Node 24 is the current Active LTS release (supported through April 2028), which is reason enough on its own.
...
Repo scaffolding (all 3 packages + CI) is done and validated per above. The
2026-08-03 backend consolidation was verified against a real Postgres 18
(podman) end to end: migrations, all 12 Route Handlers, the worker status
callback, rate-limit atomicity under concurrency, and the standalone
production build serving live requests.

Known gaps from that work, in priority order:

1. **`web/e2e/mock-backend.mjs` is stale** — it mocks a backend process that no
   longer exists, and `playwright.config.ts` still sets the now-unused
   `NEXT_PUBLIC_API_BASE_URL`. Playwright passes, but against a path production
   doesn't take. Deferred deliberately; redesign before trusting E2E coverage.
