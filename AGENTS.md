...
- **Operational scripts in `scripts/dev/` (local) and `scripts/prod/` (the deployed account) are committed executable (`100755`); helpers in `scripts/lib/` are sourced, so they stay `100644`.**
  ...
  - A script that tags the AWS resources it creates sources `scripts/lib/project-tag.sh`. It reads `local.project_tag` from `infra/locals.tf` so the tag matches `infra/providers.tf`'s `default_tags`.
  ...
  - Local Terraform is `$HOME/.local/bin/terraform` (`scripts/prod/install-terraform.sh`). `scripts/lib/terraform.sh` prefers that over PATH, because a different CLI earlier on PATH still satisfies `command -v terraform`. CI has no copy there and uses `hashicorp/setup-terraform` instead.
