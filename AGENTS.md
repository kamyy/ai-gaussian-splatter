...
- **Never put `--` before a `cdk:*` script's flags.** pnpm forwards them — the script sees `["--", "-c", "foo=bar"]` — and then cdk's own parser discards everything after the separator, so the app synthesizes against `app.py`'s placeholder context instead. It surfaces only because those placeholders are refused against a real account — `read_context` for its own four, `WebStack` for `clerkSecretKeyArn` — and the error names a flag you know you passed.
