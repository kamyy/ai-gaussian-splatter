...
- **The matcher skips static files by file extension (`.js`, `.png`, …), not by "the path contains a dot".** A page like `/objects/my.splat.v2` still needs `clerkMiddleware()` to run.
...
5. **No maximum photo count or upload size.** `MIN_PHOTOS_PER_OBJECT` has no counterpart and the presign body schema (`app/api/v1/objects/[objectId]/photos/presign/route.ts`) is `.min(1)` only. COLMAP's exhaustive matching is O(n²) pairs and the instance runs until `run_job.py` returns, so an oversized upload is unbounded GPU spend — the global daily cap in `process` bounds job count, not job cost (`ARCHITECTURE.md`).
