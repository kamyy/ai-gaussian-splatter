...
| New worker image | `scripts/prod/worker-push-image.sh` ([Building and pushing the worker image](../../../RUNBOOK.md#building-and-pushing-the-worker-image)). It also updates `WORKER_IMAGE_TAG`, and the next deploy points `WORKER_RECONSTRUCT_IMAGE_URI` and `WORKER_TRAIN_IMAGE_URI` at it. |
